...
export default function Tpresouce() {
  ...
  const ResourceCard = ({ res }) => (
    <div className="card m-2 p-3 shadow" style={{ width: "250px" }}>
      ...
      <br />
      <button className="btn btn-sm btn-outline-secondary mt-2" onClick={() => startEdit(res)}>Edit</button>
    </div>
  );
  ...
  return (
    <div className="container text-center">
      ...
      <div className="card p-3 mb-4">
        ...
        <div className="row">
          {["resourceName", "category", "uploadedBy", "driveLink", "uploadDate", "description"].map((field, idx) => (
            <div key={idx} className={field === "description" ? "col-12 mb-2" : "col-md-4 mb-2"}>
              {field === "description" ? (
                <textarea className="form-control" name={field} placeholder="Description" value={newResource[field]} onChange={handleChange} />
              ) : (
                <input
      ...
      <div className="d-flex flex-wrap">
        {resources.length > 0 ? resources.map(res => <ResourceCard key={res._id} res={res} />) : <p>No resources available.</p>}
      </div>
              ...
              <div className="modal-body">
                {["resourceName", "category", "uploadedBy", "driveLink", "uploadDate", "description"].map((field, idx) => (
                  <div className="form-group mb-2" key={idx}>
                    ...
                    {field === "description" ? (
                      <textarea className="form-control" name={field} value={editingResource[field]} onChange={(e) => handleChange(e, true)} />
                    ) : (
                      <input
                        type={field === "uploadDate" ? "date" : "text"}
                        className="form-control"
                        name={field}
                        value={field === "uploadDate" ? editingResource[field]?.split("T")[0] : editingResource[field]}
